...
export function VideoSection({ videoId }: VideoSectionProps) {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black">
      ...
      >
        {isMuted ? (
          <VolumeX className="w-6 h-6 text-umber group-hover:text-primary-600 transition-colors" aria-hidden="true" />
        ) : (
          <Volume2 className="w-6 h-6 text-umber group-hover:text-primary-600 transition-colors" aria-hidden="true" />
        )}
      </button>
    </section>
  )
